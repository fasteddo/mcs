...
namespace mame
{
    ...
    struct mat_cr_t
    {
        ...
        public uint16_t [] diag;//[N];  //C diag[N];      // diagonal index pointer n
        public uint16_t [] ia;//[N+1];  //C ia[N+1];      // row index pointer n + 1
        public uint16_t [] ja;//[N*N];  //C ja[N*N];       // column index array nz_num, initially (n * n)
        ...
        public mat_cr_t(UInt32 N, UInt32 n)
        {
            this.N = N;
