...
namespace mame.netlist.solver
{
        ...
        where int_SIZE : int_const, new()
    {
        ...
        protected override bool check_err()
        {
            ...
            size_t iN = size();
            FT reltol = ops.cast(m_params.m_reltol.op());  //const float_type reltol(static_cast<float_type>(m_params.m_reltol));
            FT vntol = ops.cast(m_params.m_vntol.op());  //const float_type vntol(static_cast<float_type>(m_params.m_vntol));
            for (size_t i = 0; i < iN; i++)
            {
                FT vold = ops.cast(this.m_terms[i].getV());  //const float_type vold(static_cast<float_type>(this->m_terms[i].getV()));
                FT vnew = m_new_V[i];  //const float_type vnew(m_new_V[i]);
                FT tol = ops.add(vntol, ops.multiply(reltol, ops.max(plib.pg.abs<FT, FT_OPS>(vnew), plib.pg.abs<FT, FT_OPS>(vold))));  //const float_type tol(vntol + reltol * std::max(plib::abs(vnew),plib::abs(vold)));
