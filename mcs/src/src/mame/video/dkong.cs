    ...
    partial class dkong_state : driver_device
    {
        ...
        ***************************************************************************/
        void draw_sprites(bitmap_ind16 bitmap, rectangle cliprect, uint32_t mask_bank, uint32_t shift_bits)
        {
            ...
            for (offs = m_sprite_bank<<9, num_sprt=0; (num_sprt < 16) && (offs < (m_sprite_bank<<9) + 0x200) /* sprite_ram_size */; offs += 4)
            {
                ...
                if (do_draw != 0)
                {
                    ...
                    m_gfxdecode.op0.gfx(1).transpen(bitmap, cliprect, (u32)code, (u32)color, flipx, flipy, x, y, 0);

                    // wraparound
                    m_gfxdecode.op0.gfx(1).transpen(bitmap, cliprect, (u32)code, (u32)color, flipx, flipy, m_flip != 0 ? x + 256 : x - 256, y, 0);
                    m_gfxdecode.op0.gfx(1).transpen(bitmap, cliprect, (u32)code, (u32)color, flipx, flipy, x, y - 256, 0);
        ...
        void scanline_callback(object ptr, int param)
        {
            ...
            m_scanline_timer.adjust(m_screen.op0.time_until_pos(scanline), scanline);
        }
                ...
                if (newset != m_vidhw)
                {
                    ...
                    switch (newset)
                    {
                        case DKONG_RADARSCP_CONVERSION:
                            radarscp_palette(m_palette.op0);
                            break;
                        case DKONG_BOARD:
                            dkong2b_palette(m_palette.op0);
                            break;
                    }
                }
            }
        ...
        void video_start_dkong()
        {
            ...
            {
                case HARDWARE_TRS02:
                    m_screen.op0.register_screen_bitmap(m_bg_bits);
                    m_gfx3 = memregion("gfx3").base_();
                    m_gfx3_len = (int)memregion("gfx3").bytes();
                    goto case HARDWARE_TKG04;  //[[fallthrough]];
                case HARDWARE_TKG04:
                case HARDWARE_TKG02:
                    m_bg_tilemap = machine().tilemap().create(m_gfxdecode.op0, dkong_bg_tile_info, tilemap_standard_mapper.TILEMAP_SCAN_ROWS,  8, 8, 32, 32);
                    break;
                case HARDWARE_TRS01:
                    m_bg_tilemap = machine().tilemap().create(m_gfxdecode.op0, radarscp1_bg_tile_info, tilemap_standard_mapper.TILEMAP_SCAN_ROWS,  8, 8, 32, 32);
