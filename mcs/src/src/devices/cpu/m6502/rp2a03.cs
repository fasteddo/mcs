...
using static mame.rp2a03_global;
using static mame.nes_apu_global;


namespace mame
{
    public class rp2a03_core_device : m6502_device
    {
        ...
        public static readonly emu.detail.device_type_impl RP2A03_CORE = DEFINE_DEVICE_TYPE("rp2a03_core", "Ricoh RP2A03 core", (type, mconfig, tag, owner, clock) => { return new rp2a03_core_device(mconfig, tag, owner, clock); });


        rp2a03_core_device(machine_config mconfig, string tag, device_t owner, uint32_t clock)
            : this(mconfig, RP2A03_CORE, tag, owner, clock)
        { }


        protected rp2a03_core_device(machine_config mconfig, device_type type, string tag, device_t owner, uint32_t clock)
    ...
    public class rp2a03_device : rp2a03_core_device
                                 //device_mixer_interface
    {
        ...
        public static readonly emu.detail.device_type_impl RP2A03 = DEFINE_DEVICE_TYPE("rp2a03", "Ricoh RP2A03", (type, mconfig, tag, owner, clock) => { return new rp2a03_device(mconfig, tag, owner, clock); });


        device_mixer_interface m_dimixer;


        required_device<apu2a03_device> m_apu;  //required_device<nesapu_device> m_apu;


        rp2a03_device(machine_config mconfig, device_type type, string tag, device_t owner, uint32_t clock)
            : base(mconfig, type, tag, owner, clock)
        {
            ...
            m_apu = new required_device<apu2a03_device>(this, "nesapu");


            program_config.m_internal_map = rp2a03_map;
        }


        rp2a03_device(machine_config mconfig, string tag, device_t owner, uint32_t clock)
            : this(mconfig, RP2A03, tag, owner, clock)
        {
        }
        ...
        void rp2a03_map(address_map map, device_t device)
        {
            ...
            map.op(0x4017, 0x4017) /*.r(FUNC(rp2a03_device::vsnes_in1_r))*/ .w(psg1_4017_w);
        }
    ...
    public static class rp2a03_global
    {
        ...
        static readonly XTAL RP2A03_NTSC_XTAL        = new XTAL(21_477_272);
        //#define RP2A03_PAL_XTAL            XTAL(26'601'712)
        public static readonly XTAL NTSC_APU_CLOCK  = RP2A03_NTSC_XTAL / 12;  /* 1.7897726666... MHz */
        ...
        public static rp2a03_device RP2A03(machine_config mconfig, string tag, XTAL clock) { return emu.detail.device_type_impl.op<rp2a03_device>(mconfig, tag, rp2a03_device.RP2A03, clock); }
    }
}
