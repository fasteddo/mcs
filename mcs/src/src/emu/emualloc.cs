...
namespace mame
{
    public static class emualloc_global
    {
        ...
        public static ListBase<T> pool_alloc_array<T>(UInt32 num) where T : new() { return global.global_alloc_array<T>(num); }  //(_pool).add_array(global_alloc_array(_type,_num), (_num))
        //#define pool_alloc_array_clear(_pool, _type, _num)  (_pool).add_array(global_alloc_array_clear<_type>(_num), (_num))
        public static ListBase<T> pool_alloc_array_clear<T>(UInt32 num) where T : new() { return global.global_alloc_array_clear<T>(num); }  // (_pool).add_array(global_alloc_array_clear<_type>(_num), (_num))
